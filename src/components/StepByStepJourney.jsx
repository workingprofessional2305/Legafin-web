...
const StepByStepJourney = () => {
  ...
  return (
    <div className="w-full bg-white py-16 px-4 sm:px-8">
      <div className="w-full mx-auto text-center mb-10 mt-2">
      ...
      <div className="overflow-hidden grid grid cols-2 sm:grid cols-2 md:grid cols-2 lg:grid cols-3 xl:grid cols-4">
        <div
          ref={scrollRef}
          className="flex gap-4 w-full mx-auto px-4 overflow-x-scroll no-scrollbar pb-2"
        >
          {steps.map((step, index) => (
            <div
              key={index}
              className={`relative group min-w-[260px] py-6 px-4 rounded-xl text-center transition-all duration-300 transform
              ${index === activeIndex ? "scale-115 bg-gradient-to-br from-[blue-200 via-green-50 to-blue-300]/90 hover:bg-blue-100" : "bg-gradient-to-br from-[blue-200 via-green-50 to-blue-300]/90 scale-100"}
